...
class FedavgServer(BaseServer):
    def __init__(self, args, writer, server_dataset, client_datasets, model):
        ...
        self.original_estimator = LearningrateEstimatorModel(self.args.lr, self.args.b1, self.args.b2, self.args.b3)
        self.adapted_lr = 0
    ...
    def _request(self, ids, eval=False, participated=False, lr_update=False):
        def __update_clients(client):
            ...
            if not self.args.drift_adaptation:
        ...
        if lr_update:
            logger.info(
                f'[{self.args.algorithm.upper()}] [Round: {str(self.round).zfill(4)}] Request lr updates to {len(ids)} clients!')
            results = []
            with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(ids), os.cpu_count() - 1)) as workhorse:
                for idx in TqdmToLogger(
                        ids,
                        logger=logger,
                        desc=f'[{self.args.algorithm.upper()}] [Round: {str(self.round).zfill(4)}] ...update lr... ',
                        total=len(ids)
                ):
                    results.append(workhorse.submit(__update_lr, self.clients[idx]).result())
            logger.info(
                f'[{self.args.algorithm.upper()}] [Round: {str(self.round).zfill(4)}] ...completed lr updates of {len(ids)} clients!')
    ...
    def _central_evaluate(self):
        ...
        # log lr
        self.writer.add_scalar('Local Learning Rate', self.client_lr, self.round)
        self.writer.add_scalar('Global Learning Rate', self.lr_scheduler.get_last_lr()[-1], self.round)
        # log TensorBoard
    ...
    def _drift_dataset(self):
        if self.args.drift_mode in ['soft', 'hard']:
            self.clients[0].training_set.subset.dataset.dataset.dataset.increase_drift_stage()
            self.server_dataset.dataset.increase_drift_stage()
    ...
    def update(self):
        ...
        """
        #plt.imshow(transforms.ToPILImage()(self.clients[0].training_set.subset.dataset.dataset.dataset[0][0]))
        #plt.show()
        if (self.args.concept_drift and self.args.drift_start <= self.round <= (
                self.args.drift_start + self.args.drift_duration)):
            self._drift_dataset()
            #plt.imshow(transforms.ToPILImage()(self.clients[0].training_set.subset.dataset.dataset.dataset[0][0]))
            #plt.show()
            #plt.imshow(transforms.ToPILImage()(self.server_dataset.dataset[0][0]))
            #plt.show()
            #a = 1
        ...
        if self.args.drift_adaptation and self.args.drift_adaptation_mode == "original":
            self.client_lr = self.adapted_lr = self.original_estimator.estimate(self.model, self.round, self.lr_scheduler.get_last_lr()[-1])

        # request update to selected clients
        updated_sizes = self._request(selected_ids, eval=False)

        #self.client_lr = np.mean([self.clients[idx].args.lr for idx in selected_ids])
