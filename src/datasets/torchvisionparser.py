...
def fetch_torchvision_dataset(args, dataset_name, root, transforms):
    ...
    if dataset_name in [
        ...
        # load custom wrapper for cifar10 with concept drift
        if dataset_name == 'CIFAR10' and args.concept_drift:
            raw_train = Cifar10ConceptDrift(total_stages=args.drift_duration, drift_mode=args.drift_mode, **train_args)
        elif dataset_name == 'MNIST' and args.concept_drift:
            raw_train = MnistConceptDrift(total_stages=args.drift_duration, drift_mode=args.drift_mode, **train_args)
        elif dataset_name == 'FashionMNIST' and args.concept_drift:
            raw_train = FashionMnistConceptDrift(total_stages=args.drift_duration, drift_mode=args.drift_mode, **train_args)
        else:
        ...
        # load custom wrapper for cifar10 with concept drift
        if dataset_name == 'CIFAR10' and args.concept_drift:
            raw_test = Cifar10ConceptDrift(total_stages=args.drift_duration, drift_mode=args.drift_mode, **test_args)
        elif dataset_name == 'MNIST' and args.concept_drift:
            raw_test = MnistConceptDrift(total_stages=args.drift_duration, drift_mode=args.drift_mode, **test_args)
        elif dataset_name == 'FashionMNIST' and args.concept_drift:
            raw_test = FashionMnistConceptDrift(total_stages=args.drift_duration, drift_mode=args.drift_mode, **test_args)
        else:
